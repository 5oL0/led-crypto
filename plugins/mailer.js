...
Mailer.prototype.setup = function(done) {
    ...
    if(mailConfig.sendMailOnStart) {
      this.mail(
        ...
        [
          ...
          '/',
          config.watch.asset,
          ". I'll let you know when I got something good to say"
        ].join(''),
        _.bind(function(err) {
  ...
  if(!mailConfig.password) {
    ...
    prompt.start();
    var warning = [
      '\n\n\tYou configured LED cryptoBOT to mail you advice, LED cryptoBOT needs your email',
...
Mailer.prototype.processStratNotification = function({ content }) {
  ...
  const text = [
    'LED cryptoBOT received new notification :\n\n',
    content
  ].join('');
